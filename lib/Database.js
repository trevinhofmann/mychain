...
// A boolean value for whether or not a block has been indexed in this session yet. Unconfirmed transactions
// should not be indexed until this is true, to avoid doubly indexing the raw mem pool.
var blockProcessedThisSession = false;
...
function addTransactions(txs, callback){
  if (unconfirmedTransactions.length > 0){
    var tempTxs = txs;
    for (var txid in tempTxs){
      if (unconfirmedTransactions.indexOf(tempTxs[txid]) != -1){
        unconfirmedTransactions.splice(unconfirmedTransactions.indexOf(tempTxs[txid]), 1);
      }
    }
  }
  if (txs.length > 0){
...
function blockAdded(callback){
  heightChecked ++;
  blockProcessedThisSession = true;
...
function processUnconfirmedTransactions(callback){
  rpc.getRawMemPool(function(err, ret){
    ...
    console.log('Processing unconfirmed transactions. Mempool size: '+mempool.length);
    var transactions = [];
    for (var txid in mempool){
      if (unconfirmedTransactions.indexOf(mempool[txid]) == -1){
        transactions.push(mempool[txid]);
      }
    }
    addTransactions(transactions, function(){
